...
public class SystemTestCommand implements ICommand{
	...
	public void execute() {
		...
		ProductInStore product10 = this.storageManager.createProductStore("Fishing Pole Model 3", 150, 320, "AT3-FD", 1);						//product to delete
		...
		this.storageManager.addQuantity(product10, 7);
		
		//creating orders for store products
		...
		this.storageManager.updateQuantity(product3, 1);
		this.orderManager.createOrder("FCHD-K4KR", customer4, product10, 3);
		this.storageManager.updateQuantity(product10, 3);
		
		//creating orders for wholesalers products
		...
		//creating orders for website products
		...
		this.storageManager.updateQuantity(product8, 2);
		...
		this.storageManager.updateQuantity(product9, 1);
		...
		this.storageManager.updateQuantity(product9, 3);
		
		Order orderToUndo = this.orderManager.createOrder("FCHD-K44R", customer5, product10, 4);
		this.storageManager.updateQuantity(product10, 4);
		
		//deleting a product
		this.storageManager.setProductActivity(product10.getProductID(), false);
		this.storageManager.setProductStock(product10.getProductID(), 0);
		
		//undoing last order
		System.out.println("Dear " + orderToUndo.getCustomer().getCustomerName() + ",");
		System.out.println("We are sorry to inform you that your order " + orderToUndo.getOrderID() + " has been canceled due to storage issues.");
		this.orderManager.removeLastOrder();
		this.storageManager.addQuantity(this.storageManager.getProductByID(orderToUndo.getProductID()), orderToUndo.getQuantity());
		System.out.println("\n\n");
		
		//printing the information of the store's products and orders
		int profit = 0;
		int spendings = 0;
		...
		for (Iterator<Order> iterator = orderManager.getAllOrders().iterator(); iterator.hasNext();) {
			...
			if(order instanceof OrderShipped && ((OrderShipped) order).getShippingType().equals(eShippingType.Standard)) {
